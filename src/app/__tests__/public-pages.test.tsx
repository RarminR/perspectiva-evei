...
describe('Case studies listing page (/studii-de-caz)', () => {
  it('renders "Studii de Caz" heading', async () => {
    const { prisma } = await import('@/lib/db')
    ...
    render(jsx)
    expect(screen.getByText('Studii de Caz')).toBeInTheDocument()
  })

  it('shows placeholder when no case studies', async () => {
    const { prisma } = await import('@/lib/db')
    vi.mocked(prisma.caseStudy.findMany).mockResolvedValue([])
    ...
    const jsx = await CaseStudiesPage()
    render(jsx)
    expect(screen.getByText(/Studiile de caz vor fi disponibile în curând/i)).toBeInTheDocument()
  })

  it('renders case study cards when data exists', async () => {
    const { prisma } = await import('@/lib/db')
    vi.mocked(prisma.caseStudy.findMany).mockResolvedValue([
      {
        id: '1',
        title: 'Povestea Mariei',
        slug: 'povestea-mariei',
        content: 'Transformare completă',
        coverImage: null,
        testimonialQuote: 'Mi-a schimbat viața!',
        clientName: 'Maria',
        published: true,
        createdAt: new Date(),
        updatedAt: new Date(),
    ...
    const jsx = await CaseStudiesPage()
    render(jsx)
    expect(screen.getByText('Povestea Mariei')).toBeInTheDocument()
    expect(screen.getByRole('link', { name: /Citește povestea/i })).toBeInTheDocument()
  })
})
